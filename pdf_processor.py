...
def fetch_chunks(retriever, part_number, attr_key, k=8):
    ...
    dense_results = retriever.get_relevant_documents(attr_key)[:k]# Récupère les documents les plus pertinents pour l'attribut
    ...
    logger.debug(f"Retrieved {len(dense_results)} initial chunks for attribute '{attr_key}'")#Affiche le nombre de chunks récupérés initialement
    ...
    for i, chunk in enumerate(dense_results):#Affiche les métadonnées de chaque chunk pour le débogage
        logger.debug(f"Chunk {i+1} metadata: {chunk.metadata}")
    
    filtered = []#Liste vide qui va contenir les chunks filtrés
    for chunk in dense_results:#Pour chaque chunk récupéré
